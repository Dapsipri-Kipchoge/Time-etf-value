...
def run(cfg):
    ...
    for c in ("유효PEG", "fPER", "fPOR", "fPER(사이트)", "PEG(매출)1y", "PEG(영익)1y", "PEG(영익)2y", "ROE(E)", "영업이익률(E)", "매출증가율1y", "탈락조건", "밸류점수" if "밸류점수" in df.columns else "fPER"):
        if c not in df.columns: df[c] = "" if c == "탈락조건" else float("nan")
        if c != "탈락조건": df[c] = pd.to_numeric(df[c], errors="coerce")
    if df["fPER"].notna().sum() == 0:
        st.error(f"수집 데이터에 지표가 비어 있음 — 수집 실패 상태. GitHub Actions 로그에서 종목별 '실패:' 메시지를 확인하세요. (기준일 {df['기준일'].iloc[0] if '기준일' in df.columns else '?'})")
    ...
    with T[4]:
        try:
            ...
            for c in ("유효PEG", "fPER", "fPOR", "PEG(매출)1y", "PEG(영익)1y", "PEG(영익)2y", "ROE(E)", "영업이익률(E)", "전년영업이익률", "매출증가율1y", MC):
